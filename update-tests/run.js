...
//   cd card-tests && npm ci && cd .. && node update-tests/run.js
...
const req = (name) => require(require.resolve(name, { paths: [path.join(ROOT, 'card-tests')] }));
const { JSDOM, VirtualConsole } = req('jsdom');
const JQUERY = fs.readFileSync(require.resolve('jquery/dist/jquery.js', { paths: [path.join(ROOT, 'card-tests')] }), 'utf8');
