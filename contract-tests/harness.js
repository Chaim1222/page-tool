...
function makeNetwork(scenario, onCall) {
  ...
  function ajaxOutcome(call) {
    ...
    if (key.startsWith('wp:log:')) return { data: buildLogs(scenario, p.letitle, p.letype) };
...
function buildLogs(s, title, type) {
  const byTitle = (s.logs && s.logs[title]) || {};
  return { query: { logevents: (byTitle[type] || []).map((x) => ({ ...x, type })) } };
}
