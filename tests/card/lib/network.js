...
function classify(endpoint, p) {
  ...
  if (endpoint === WP) {
    ...
    if (p.list === 'logevents') {
      return { route: 'wp-log', key: `wp-log:${norm(p.letitle)}:${p.letype}`, title: norm(p.letitle), type: p.letype, params: p };
    }
...
function createNetwork(options) {
  ...
  function wpLog(info) {
    ...
    return { data: { query: { logevents: logRange(events, info.params) } } };
  }
